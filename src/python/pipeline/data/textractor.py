...
class Textractor(Segmentation):
    ...
    def text(self, text):
        ...
        # Debug print
        print(f"Processing input. Is file: {exists}, Path: {path}, Text length: {len(text) if text else 0}")
        ...
        if not path:
            ...
            print(f"Using HTML processor with backend: {self.html.backend if hasattr(self.html, 'backend') else 'None'}")
